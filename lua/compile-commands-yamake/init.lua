...
local function generate_compile_commands(root, with_codegen)
	...
	vim.system({ "ya", "dump", "compile_commands", "--force-build-depends" }, function(obj)
...
local function make_autocmd_callback(opts)
	...
	return function()
		...
		elseif opts.mode == "ask" then
			vim.ui.input({ prompt = "Generate compile_commands.json for " .. root .. "? [Y/n] " }, function(input)
...
function M.setup(opts)
	...
	vim.api.nvim_create_user_command("GenerateCompileCommands", function(_)
		...
		generate_compile_commands(root, true)
	end, { desc = "Generate compile_commands.json via ya dump compile_commands" })
